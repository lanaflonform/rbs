...
export const rbsController = ng.controller('RbsController', [
    ...
    ($location, $scope, $timeout, $compile, $sanitize, model, route) => {
        ...
        // Navigation
        $scope.showCalendar = async function (refresh) {
            if (refresh === true) {
              await $scope.initMain();
            }
        ...
        $scope.showList = async function (refresh) {
            ...
            await $scope.bookings.sync(true, $scope.resources);
            $scope.bookings.filters.startDate = moment($scope.bookings.filters.startDate);
            $scope.bookings.filters.endDate =
                moment($scope.bookings.filters.startDate).add(2, 'month').startOf('day');
        ...
        $scope.showBookingDetail = function (booking, slot, displaySection) {
            let calendarSlot = _.findWhere($scope.bookings.all,{id: booking.parent_booking_id});
            if(calendarSlot) {
        ...
        $scope.switchSelectAllSlots = function (booking) {
            // bookings.all.forEach((booking) => {
                if (booking.is_periodic === true && booking.selected === true) {
                    _.each(booking._slots, function (slot) {
                        slot.selected = true;
                        $scope.bookings.selectedElements.push(slot);
                    });
        ...
        $scope.checkStatus = function (bookings) {
                return _.every(bookings, function(booking) {
                    if (booking && booking.status != 1 && booking.status != null) {
                        if (booking.isSlot() && booking.parent_booking_id == null) {
                            booking.status = 9;
                        }
        ...
        $scope.countValidatedSlots = function (_slots) {
            return _.filter(_slots, function (slot) {
        ...
        $scope.autoSelectResource = async function () {
            ...
            $scope.booking.resource = $scope.booking.resourceType === undefined ? undefined
                : _.first($scope.booking.resourceType.resources.filterAvailable($scope.booking, $scope.booking.is_periodic));
        ...
        $scope.resolveSlotsSelected = function (start, end) {
            let hasErrors = false;
            if (start <= end) {
        ...
        $scope.removeBookingSelection = function () {
            ...
            _.each($scope.bookings.selected, function(booking) {
                if (!$scope.isViewBooking) {
                    $scope.currentBookingSelected = booking;
                }
                if (
                    booking.isSlot() &&
                    booking.occurrences !== booking.slots.all.length
                ) {
                    ...
                    booking._slots.forEach(slot => slot.selected = true);
                }
            });

            function displayMessage() {
                $scope.booking = $scope.bookings.selected;
                if (
                    $scope.currentBookingSelected.isSlot() &&
                    $scope.currentBookingSelected.slots.length !== 1
                ) {
                    $scope.isViewBooking = false;
            ...
            if (totalSelectionAsynchroneCall === 0) {
                ...
                _.each($scope.bookings.selected, async function (booking) {
                    if (!$scope.isViewBooking) {
                        $scope.currentBookingSelected = booking;
                    }
                    if (booking.isSlot() && booking.occurrences !== booking.slots.all.length) {
        ...
        $scope.showConfirmDeleteMessage = function () {
            $scope.processBookings = $scope.selectionForProcess($scope.booking);
        ...
        $scope.doRemoveBookingSelection = async function () {
            ...
            try {
                if ($scope.booking instanceof Array) {
                    const promises = [];
                    $scope.booking.forEach((booking) => promises.push(booking.delete()));
        ...
        $scope.doRemoveCurrentAndFuturBookingSelection = async function () {
            $scope.display.processing = true;
            $scope.currentErrors = [];
            try {
                await $scope.booking.delete();
        ...
        $scope.selectionForDelete = function () {
            return _.filter($scope.bookings.selected, function (booking) {
                return booking.isBooking();
            });
        };
        ...
        $scope.doValidateBookingSelection = function () {
            ...
            $scope.currentErrors = [];
            try {
                _.each($scope.processBookings, async function (booking) {
        ...
        $scope.editSelectedType = function () {
            ...
            template.close('resources');
            $scope.updateSlotProfileField($scope.editedResourceType.school_id);
            $scope.editedResourceType._id = $scope.editedResourceType.id;
            template.open('resources', 'edit-resource-type');
        };
